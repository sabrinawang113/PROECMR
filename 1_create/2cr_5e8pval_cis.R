################################################################################
#	R-script:		  2cr_5e8pval_cis.R
#	Project:	  	PROECMR : Protein and EC Risk - Mendelian randomisation
#
#	Data used:		2_derivedata/proteins_p5e8
#
#	Data created:	2_derivedata/proteins_instrument_r001
#
# Purpose:  		select cis (or trans) variants as iv for each protein
#               LD clump to select independent snps
#               _r001 : r<0.01 for cis variants; r<0.001 for trans variants
################################################################################

## set file path
rm(list = ls(all.names = TRUE))
INPUT <- "~/_PROJECTS/Proj_PROECMR_osiris/2_derivedata/proteins_p5e8"
OUTPUT <- "~/_PROJECTS/Proj_PROECMR_osiris/2_derivedata/proteins_instrument_r001"
...
for (file_name in file_list) {
  ...
  gene_start <- subset(data_genepos$gene_start, data_genepos$Assay == olk_name)
  gene_end <- subset(data_genepos$gene_end, data_genepos$Assay == olk_name)
...
write.table(ivsnps_clump,  file = "list_proteins_instrument_r001.txt")
